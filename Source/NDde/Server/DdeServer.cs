...
///     </para>
///     <para>
///         After this object has registered its service name by calling the <c>Register</c> method clients can connect to
///         it by specifying the service
///         name the server registered and a topic name that it supports.
///     </para>
///     <para>
///         Event methods are invoked on the thread hosting the <c>DdeContext</c>.  All operations must be marshaled onto
///         the thread hosting the
...
public abstract class DdeServer : IDisposable
{
    ...
    /// <summary>
    ///     This initializes a new instance of the <c>DdeServer</c> class that can register the specified service name and
    ///     using the specified
    ...
    /// <summary>
    ///     This initializes a new instance of the <c>DdeServer</c> class that can register the specified service name and uses
    ///     the specified
    ///     context.
    /// </summary>
    /// <param name="service">
    ...
    {
        private readonly DdeServer _Parent;

        public MyDdemlServer(string service, DdemlContext context, DdeServer parent) : base(service,
            context)
        {
            _Parent = parent;
        }
        ...
        {
            var result = _Parent.OnExecute((DdeConversation) conversation.Tag, command);
        ...
        protected override PokeResult OnPoke(DdemlConversation conversation, string item, byte[] data,
            int format)
        {
            var result = _Parent.OnPoke((DdeConversation) conversation.Tag, item, data, format);
        ...
        protected override RequestResult OnRequest(DdemlConversation conversation, string item,
            int format)
        {
            var result = _Parent.OnRequest((DdeConversation) conversation.Tag, item, format);
